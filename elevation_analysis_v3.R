...
hu_hts_avg <- elev %>%
  ...
  dplyr::select(site, z, zmin, zmaxn, zmeann, zmean_raw
                ) %>%
  transmute(site = site,
            ht_abs_mean = zmean_raw,
            ht_abs = z,
            ht_rel = zmaxn,
            ht_mean = zmeann
            ) %>%
...
elev_det <- elev %>%
  ...
  dplyr::select(mean, median, quant_80, 
                quant_20, z, z_mod_lin, site) %>%
  transmute(mean = mean + z_mod_lin,
            median = median + z_mod_lin,
            quant_80 = quant_80 + z_mod_lin,
            quant_20 = quant_20 + z_mod_lin,
            site = site) %>%
...
df_hu <- elev %>%
  dplyr::filter(is.na(point),
                between(area, 0.2, 3)) %>%
  mutate(dist_hydmedian = zmin_raw - median,
